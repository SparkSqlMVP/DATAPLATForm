	...
	label: function (str, x, y, shape, anchorX, anchorY, useHTML, baseline, className) {
		...
		return extend(wrapper, {
			...
			css: function (styles) {
				if (styles) {
					var textStyles = {};
					styles = merge(styles); // create a copy to avoid ing the original object (#537)
...
Axis.prototype = {
	...
	render: function () {
		var axis = this,
			...
			nateBands = axis.nateBands,
			stackLabelOptions = options.stackLabels,
			nateGridColor = options.nateGridColor,
			tickmarkOffset = axis.tickmarkOffset,
		...
		each([ticks, minorTicks, nateBands], function (coll) {
			var pos;
			for (pos in coll) {
			...
			if (nateGridColor) {
				each(tickPositions, function (pos, i) {
					if (i % 2 === 0 && pos < axis.max) {
						if (!nateBands[pos]) {
							nateBands[pos] = new PlotLineOrBand(axis);
						}
						from = pos + tickmarkOffset; // #949
						to = tickPositions[i + 1] !== UNDEFINED ? tickPositions[i + 1] + tickmarkOffset : axis.max;
						nateBands[pos].options = {
							from: isLog ? lin2log(from) : from,
							to: isLog ? lin2log(to) : to,
							color: nateGridColor
						};
						nateBands[pos].render();
						nateBands[pos].isActive = true;
					}
				});
			}
		...
		each([ticks, minorTicks, nateBands], function (coll) {
			...
			if (coll === nateBands || !chart.hasRendered || !delay) {
	...
	destroy: function (keepEvents) {
		...
		// Destroy collections
		each([axis.ticks, axis.minorTicks, axis.nateBands], function (coll) {
...
Pointer.prototype = {
	...
	pinchTranslateDirection: function (horiz, pinchDown, touches, transform, selectionMarker, clip, lastValidTouch) {
		...
		selectionXY = clipXY; // the clip position (x or y) is ed if out of bounds, the selection position is not
...
defaultPlotOptions.area = merge(defaultSeriesOptions, {
	threshold: 0
	// trackByArea: false,
	// lineColor: null, // overrides color, but lets fillColor be uned
	// fillOpacity: 0.75,
	// fillColor: null
});
